...
object ZFlow {
  type ZFlowEnv = Has[ZFlow.Service]

  trait Service extends ZioMongoExecutor[Flow] {
    ...
    def save(command: CreateFlowCommand): Task[Boolean]

    def update(id: String, command: CreateFlowCommand): Task[Int]
  ...
  val live: ZLayer[Any, Nothing, ZFlowEnv] = ZLayer.succeed(new Service() {
  ...
  def list(): ZIO[ZFlowEnv, Throwable, Vector[FlowItem]] = ZIO.accessM(_.get.list())

  def list(startDate: Option[String], endDate: Option[String], platform: Option[String], category: Option[String]): ZIO[ZFlowEnv, Throwable, Vector[FlowItem]] = ZIO.accessM(_.get.list(startDate, endDate, platform, category))

  def save(command: CreateFlowCommand): ZIO[ZFlowEnv, Throwable, Boolean] = ZIO.accessM(_.get.save(command))

  def update(id: String, command: CreateFlowCommand): ZIO[ZFlowEnv, Throwable, Int] = ZIO.accessM(_.get.update(id, command))
